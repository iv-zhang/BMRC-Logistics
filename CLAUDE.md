...
- Pack `status` is **derived** on check-in/audit (not hardcoded `Ready`): expired/reported → `Expired Items`; a required consumable at **0** (not restocked), uncounted, lapsed life-safety asset, or sharps full → `Restock Needed`; else `Ready`. Checkout → `In Use`. A **partly-short** item (e.g. 1 of 2) does **not** block `Ready` — it shows as a "Needs restock · N" chip derived from `contents[].currentQuantity` by `getPackShortages` (`app/lib/statpack-shortages.ts`). Admin/QM can override a not-ready (non-expired, not checked-out) pack to `Ready` via `overrideStatpackReady` → `Statpack.readyOverride` stamp; **every check-off clears it** (one-shot). See decisions.md D-8 amendment.
