...
async function main() {
  ...
  snap.forEach(doc => {
    ...
    if (JSON.stringify(normalized) !== JSON.stringify(data)) operations.push({ id: doc.id, normalized });
  });
  if (operations.length === 0) { console.log('No changes required.'); return; }
  console.log('Will update', operations.length, 'documents.');
  if (!argv.includes('--force')) { console.log('Run again with --force to apply changes.'); process.exit(0); }
