      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="mb-4 md:mb-6">
          <div className="flex items-center justify-between">
            <h1 className="text-xl md:text-3xl font-semibold flex items-center gap-3">
          ...
          <p className="text-sm md:text-base text-foreground-500 mt-2">
        ...
        <section>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">Statpacks</h2>
                  ...
                  >
                    ...
                    <CardBody className="pt-2">
                      ...
                      {pack.currentLocation && (
                        <div className="text-xs text-foreground-400 mb-1">
                      ...
                      {/* EXPANDED CONTENT */}
                      {isExpanded && (
                        <div className="mt-4 pt-4 border-t border-divider animate-appearance-in">
                          <div className="flex justify-between items-end mb-2">
                            <h4 className="text-xs font-semibold text-foreground-400 uppercase">Recent Activity</h4>
                            <Button
                              size="sm"
                              variant="light"
                              color="primary"
                          ...
                          <div className="bg-content2 rounded-lg p-2 min-h-[100px]">
                            {isLoadingLogs ? (
                              ...
                              <ScrollShadow className="max-h-[150px]">
                                <ul className="space-y-3">
                                  {logs.map((log) => (
                                    <li key={log.id} className="border-b border-divider pb-2 last:border-0">
                                      <div className="flex justify-between items-start">
                                        ...
                                        <span className="text-xs text-foreground-400 whitespace-nowrap ml-2">
                                          {log.timestamp ? (log.timestamp instanceof Date ? log.timestamp.toLocaleString() : new Date(log.timestamp as any).toLocaleString()) : '—'}
                                        </span>
                                      </div>
                                      {log.notes && (
                                        <p className="text-xs text-foreground-500 mt-1 break-words italic">
                              ...
                              <p className="text-xs text-center text-foreground-400 py-4">
                      ...
                      <div className="mt-2 flex justify-center">
                        {isExpanded ? (
                          <ChevronUp className="text-foreground-400 transition-colors" size={16} />
                        ) : (
                          <ChevronDown className="text-foreground-400 transition-colors" size={16} />
                        )}
                      </div>
                    </CardBody>
                  </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="h-full shadow-md border border-divider">
            <CardHeader className="flex gap-3 bg-danger-50/70 dark:bg-danger-900/20 border-b border-danger-100 dark:border-danger-900/30 px-6 py-4">
              <div className="p-2 bg-danger-100 dark:bg-danger-800 rounded-lg text-danger dark:text-danger-100">
                <AlertTriangle size={18} />
              </div>
              <div>
                <h3 className="text-lg font-semibold">Supply Closet Low</h3>
                  ...
                  {lowStockItems.map((item) => (
                    <div key={item.id} className="flex justify-between items-center p-4">
                      <div>
                        ...
                        <p className="text-xs text-foreground-500">Category: {item.category}</p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-center gap-2 justify-end">
                           <span className="text-danger font-semibold tabular-nums">{item.totalStockQuantity}</span>
                           <span className="text-foreground-400 text-sm">/ {item.reorderThreshold}</span>
                        </div>
                        <p className="text-xs text-danger font-medium">Reorder Needed</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Card className="h-full shadow-md border border-divider">
             <CardHeader className="flex gap-3 bg-warning-50/70 dark:bg-warning-900/20 border-b border-warning-100 dark:border-warning-900/30 px-6 py-4">
              <div className="p-2 bg-warning-100 dark:bg-warning-800 rounded-lg text-warning dark:text-warning-100">
                <Clock size={18} />
              </div>
              <div>
                    ...
                    <div key={idx} className="flex justify-between items-center p-4">
                      <div>
                        <div className="flex items-center gap-2">
                           ...
                           <span className={`text-xs px-1.5 py-0.5 rounded ${alert.source === 'kit' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-semibold tabular-nums ${alert.daysRemaining < 0 ? 'text-danger' : 'text-warning'}`}>
