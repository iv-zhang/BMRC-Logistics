...
export interface BuyListPanelProps {
  /** 'mine' filters items where addedBy === current user uid. Default 'all'. */
  viewMode?: 'all' | 'mine';
  /** When false, omit the internal page title/Add button header. Default true. */
  showHeader?: boolean;
  /** Controlled add-modal open state (for an external Add button, e.g. the committee board). */
  addOpen?: boolean;
...
export default function BuyListPanel({
  ...
  // Add/Edit modal — supports both an internal trigger (own Add button) and
  // external control via addOpen/onAddOpenChange (committee board's Add button).
  const internalModal = useDisclosure();
  const isControlled = addOpen !== undefined && onAddOpenChange !== undefined;
  const modalIsOpen = isControlled ? !!addOpen : internalModal.isOpen;
  const modalOnOpenChange = isControlled ? (onAddOpenChange as (open: boolean) => void) : internalModal.onOpenChange;
  const openAddModal = () => (isControlled ? onAddOpenChange!(true) : internalModal.onOpen());
  const closeAddModal = () => (isControlled ? onAddOpenChange!(false) : internalModal.onClose());
  ...
  const filteredItems = useMemo(() => {
    ...
    // Mine filter (driven by the shared All/Mine toggle when embedded)
    if (viewMode === "mine" && user) {
      result = result.filter((i) => i.addedBy === user.uid);
    }
    ...
    return result;
  }, [items, viewMode, user, statusFilter, priorityFilter, searchQuery, sortField]);
  ...
  if (loading) {
    return (
      <div className="space-y-4 md:space-y-6">
        <Card><CardBody className="text-center py-12 text-foreground-400 text-sm">Loading buy list…</CardBody></Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {/* Header (omitted when embedded, e.g. in the Committee Board) */}
      {showHeader && (
      ...
      {/* Add/Edit Modal */}
      <Modal isOpen={modalIsOpen} onOpenChange={modalOnOpenChange} size="lg">
        <ModalContent>
